...
Convention
----------
The spectrum pipeline uses the emitted-field / ``P^+(t)`` convention. The
detection-time transform therefore keeps the sign

    S(omega_det) = integral dt_det E(t_det) exp(-i omega_det t_det),

implemented numerically with ``fft``. Along the coherence axis, rephasing uses
the ``+i`` branch and nonrephasing uses the ``-i`` branch, so the current
Fourier directions are

    rephasing:    ifft along t_coh,
    nonrephasing: fft  along t_coh.
"""
...
def compute_spectra(
    ...
    *,
    ...
    Based on the paper: https://doi.org/10.1063/1.2200704
    Convention note:
