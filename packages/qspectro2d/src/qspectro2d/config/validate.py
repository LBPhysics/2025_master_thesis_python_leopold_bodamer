...
def _bath_exponent_for_type(bath_type: str, raw_value: float | None) -> float | None:
    if raw_value is not None:
        return raw_value
    if bath_type.startswith("subohmic"):
        return 0.8
    if bath_type.startswith("superohmic"):
        return 1.2
...
def validate_config(cfg: Mapping[str, Any]) -> None:
    ...
    bath_s = _bath_exponent_for_type(bath_type, bath_cfg.get("s"))
    ...
    } and (bath_s is None or float(bath_s) <= 0):
