...
def estimate_slurm_resources(
    ...
    *,
    ...
    if base_t_override is not None:
        ...
        time_per_combo = base_t * solver_factor[solver] * rwa_factor * (n_times / 1000) * (N_dim**2)

    # total time for one batch (divide by workers)
    time_safety = 2.5
    base_time = 600.0  # 10 mins
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="GPGPU,metis",
        help="SLURM partition(s) to target",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(f"Requested resources: mem={requested_mem}, time={requested_time}, cpus=16")
