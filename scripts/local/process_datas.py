...
def _group_by_sample(entries: list[RunEntry]) -> dict[int, list[RunEntry]]:
    ...
    for entry in entries:
        sample_idx = entry.metadata["sample_index"]
...
def _stack_group_to_2d(group: list[RunEntry]) -> RunEntry:
    ...
    ref_t_coh = reference.metadata["t_coh_value"]
    ...
    for entry in group_sorted:
        entry_t_coh = float(entry.metadata["t_coh_value"])
        if collapsed and np.isclose(
            entry_t_coh, float(collapsed[-1].metadata["t_coh_value"]), atol=tol
        ):
    ...
    # Stack signals - all have same t_det shape now
    stacked_signals = {
        sig: np.stack([entry.signals[sig] for entry in collapsed], axis=0) for sig in signal_types
    }

    t_coh_values = [entry.metadata["t_coh_value"] for entry in collapsed]
