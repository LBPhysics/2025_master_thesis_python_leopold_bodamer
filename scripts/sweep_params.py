...
def build_ofat_cases(base_cfg: dict[str, Any]) -> list[SweepCase]:
    base_params = {
        ...
        "config.t_coh": base_cfg["config"].get("t_coh", 30.0),
        "config.t_wait": base_cfg["config"].get("t_wait", 10.0),
        "config.dt": base_cfg["config"].get("dt", 0.1),
    }
    ...
    grids: dict[str, list[Any]] = {
        ...
        "config.n_inhomogen": [1],#, 5, 10],
        "config.t_coh": [10.0],#, 20.0, 30.0],
        "config.t_wait": [20.0],#, 50.0, 100.0],
        "config.dt": [0.1, 0.2, 1.0],
    }
...
def main() -> None:
    ...
    env = build_env()
    calc_datas = SCRIPTS_DIR / "calc_datas.py"
